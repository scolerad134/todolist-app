...
type User struct {
	...
	Version     int
	FullName    string
	PhoneNumber *string //Этот параметр указатель потому, что он опциональный. Указывает на nil, если номер не задан.
}
...
func (u *User) Validate() error {
	...
	if u.PhoneNumber != nil {
		...
		if !re.MatchString(*u.PhoneNumber) {
			return fmt.Errorf("invalid PhoneNumber format: %d: %w",
				phoneNumberLength,
				core_errors.ErrInvalidArgument)
		}
	}
